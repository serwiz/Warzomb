...
socket.on("update", function(data) {
  for (var i = 0; i < data.player.length; i++) {
    ...
    if (player) {
      // position
      if (data.player[i].x !== undefined && Option.list[player.room] !== undefined && Option.list[player.room].start) {
        if (data.player[i].direction === 1) {
          if (!player.inAction && data.player[i].x !== player.x) {
            ...
            else if (player.frameX < 12 && player.class === "archer")  player.frameX++;
            else {
              player.frameX = 0;
      ...
      if (data.player[i].y !== undefined && Option.list[player.room] !== undefined && Option.list[player.room].start) {
  ...
  if (data.enemy)
    ...
    for (var i = 0; i < data.object.length; i++) {
      ...
      if (o) {
        ...
        if (data.object[i].property !== undefined) o.property = data.object[i].property;
 ...
 */
 socket.on("skill", function(data) {
  if (!clientPlayer.list[data.user].inAction && !clientPlayer.list[data.user].useSkill && clientPlayer.list[data.user].class !== "archer") {
    clientPlayer.list[data.user].inAction = true;
...
socket.on("gameOver", function(data) {
  ...
  } else if (clientPlayer.list[playerId].room === data.room &&
